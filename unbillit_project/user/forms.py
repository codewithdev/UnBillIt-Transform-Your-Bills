...
class RegistrationForm(FlaskForm):
    first_name = StringField('First Name', validators=[DataRequired(), Length(min=2, max=24, message='First name must be 2 to 24 characters long')])
    last_name = StringField('Last Name', validators=[DataRequired(), Length(min=2, max=24, message='Last name must be 2 to 24 characters long')])
    email = StringField('Email', validators=[DataRequired(), Email()])
